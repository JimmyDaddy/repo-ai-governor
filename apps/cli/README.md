...
12. `host export/verify/pack` 负责 staged host assets、verification summary 与 pack receipt；公开 host family 已覆盖 `codex`、`claude-code` 与 `github-copilot`，其中 `github-com-agent` 仍保持 reserved/fail-closed；公开的 service-host 根包入口固定为 `repo-ai-governor/service-host`。
