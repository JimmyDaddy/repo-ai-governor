...
test("review creates a pending CR file with warning findings for TODO markers and missing tests", async () => {
  ...
  assert.ok(payload.findings.some((finding: any) => finding.id.startsWith("review.todo-marker")));
  assert.ok(payload.findings.some((finding: any) => finding.id.startsWith("review.mirrored-test")));
...
test("review fails when sprint task records are out of sync", async () => {
  ...
  assert.ok(payload.findings.some((finding: any) => finding.id === "review.task-record-sync"));
});
