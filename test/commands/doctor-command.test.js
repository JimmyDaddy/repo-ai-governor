...
async function bootstrapRepository(cwd) {
...
test("doctor fails when the main config file is missing", async () => {
  ...
  const payload = JSON.parse(result.stdout);
  const configCheck = payload.checks.find((check) => check.id === "config.main-file");
...
test("doctor strict fails on missing artifact warnings and --fix recreates safe directories", async () => {
  ...
  const fixedChecks = fixPayload.checks.filter((check) => check.status === "fixed");
