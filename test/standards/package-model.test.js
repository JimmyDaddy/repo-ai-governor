...
test("createStandardsRule and renderRuleView provide AI and human consumable shapes", () => {
  ...
  assert.equal(aiView.instruction, "Use Conventional Commit format when generating commit messages.");
  assert.equal(aiView.blockOnViolation, true);
  assert.equal(humanView.summary, "提交信息必须遵循 Conventional Commit。");
  assert.equal(humanView.rationale, "这样便于发布与变更归类。");
});
