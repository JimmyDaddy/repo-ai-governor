...
describe("public package exports audit", () => {
  ...
  it("keeps repo consumers on root imports instead of deep package subpaths", () => {
    const auditFiles = [
      ...IMPORT_SCAN_ROOTS.flatMap((rootPath) => collectAuditFiles(resolve(process.cwd(), rootPath))),
      ...IMPORT_SCAN_FILES.flatMap((filePath) => collectAuditFiles(resolve(process.cwd(), filePath))),
    ];
