...
test("standard workflow template exposes the expected serial stage sequence", () => {
  assert.deepEqual(listWorkflowStageIds(STANDARD_WORKFLOW_TEMPLATE), STANDARD_WORKFLOW_STAGE_SEQUENCE);
...
test("resolveWorkflowTemplate applies workflow config overrides onto the standard template", () => {
  ...
  assert.equal(resolvedTemplate.stages.find((stage) => stage.id === "implement")?.onFailure, "warn");
  assert.equal(resolvedTemplate.stages.find((stage) => stage.id === "review")?.requiresApproval, true);
});
