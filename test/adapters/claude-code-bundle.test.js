...
test("Claude Code bundle renders system and task prompts for the current sprint", async () => {
  ...
  assert.equal(bundle.entry.agentEntry.path, "AGENTS.md");
  assert.equal(bundle.entry.currentContext.path, ".repo-ai-governor/context/current-context.md");
  ...
  assert.ok(bundle.entry.agentEntry.excerpt);
