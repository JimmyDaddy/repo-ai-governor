...
test("resolveSkillPackageLayout exposes bundled roots and install targets", () => {
  ...
  assert.equal(
    layout.absolute.catalogFile,
    "/workspace/repo-ai-governor/skills/official/catalog.json",
  );
...
test("resolveSkillPackageLayout can compute per-skill paths", () => {
  ...
  assert.equal(layout.relative.templatesDir, "skills/official/governor-plan-runner/templates");
  assert.equal(
