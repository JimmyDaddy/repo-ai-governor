  ...
  rules: [
    ...
    },
    {
      ...
      localizedTemplates: {
        "zh-CN": {
          [StandardsRenderTarget.HUMAN]:
            "交付前执行 `go fmt ./...`，保持格式与 import 排列一致。",
          [StandardsRenderTarget.AI]:
            "Run `go fmt ./...` before delivery.",
          [StandardsRenderTarget.AGENTS]:
            "Require `go fmt ./...` before delivery.",
        },
        "en-US": {
          [StandardsRenderTarget.HUMAN]:
    ...
    },
    {
      ...
      localizedTemplates: {
        "zh-CN": {
          ...
          [StandardsRenderTarget.AI]:
            "Run `go test ./...` and add coverage for changed behavior.",
          [StandardsRenderTarget.AGENTS]:
