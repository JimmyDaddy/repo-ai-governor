...
export class GithubCopilotAgentAdapter extends AgentProtocol {
   ...
   */
  private async resolveProbeResolution(): Promise<GithubCopilotProbeResolution> {
    if (this.options.executionMode === GithubCopilotAgentAdapterExecutionMode.BASELINE) {
      return {
   ...
   * @returns Probe availability resolution.
   */
  private async executeHealthProbe(): Promise<GithubCopilotProbeResolution> {
    try {
      const executionResult = await this.runGithubCopilotOperation({
      ...
      if (parsedOutput.responseText.trim() !== GITHUB_COPILOT_HEALTH_CHECK_EXPECTED_RESPONSE) {
        return {
          availabilityStatus: AgentAvailabilityStatus.UNAVAILABLE,
          unavailableReasons: [
            `health_check_invalid_response:${GITHUB_COPILOT_SURFACE}:${this.sanitizeReasonSegment(parsedOutput.responseText)}`,
          ],
        };
      }
  ...
  private resolveProbeFailureReasons(error: unknown): string[] {
    ...
    return [
      `health_check_failed:${GITHUB_COPILOT_SURFACE}:${this.sanitizeReasonSegment(standardizedError.message)}`,
    ];
  }
