...
interface ResolvedWebhookNotificationProviderOptions {
  providerId: string;
  endpointUrl: string;
...
/**
 * Sends HITL notification payloads to one generic webhook endpoint.
 */
export class WebhookNotificationProvider implements NotificationProvider {
  public readonly providerId: string;
  public readonly channel = NotificationChannel.WEBHOOK;
  private readonly resolvedOptions: ResolvedWebhookNotificationProviderOptions;

  public constructor(options: WebhookNotificationProviderOptions) {
    this.resolvedOptions = this.resolveOptions(options);
    this.providerId = this.resolvedOptions.providerId;
  }

  /**
   * Sends one notification request to the configured webhook endpoint.
  ...
  private resolveOptions(
  ...
  ): ResolvedWebhookNotificationProviderOptions {
    if (!options || typeof options !== "object") {
      throw new RuntimeError(
        GovernorErrorCode.NOTIFICATION_DISPATCH_INPUT_INVALID,
        "WebhookNotificationProvider requires one non-null options object.",
      );
    }

    const providerId = this.readRequiredString(
      options.providerId ?? WEBHOOK_NOTIFICATION_DEFAULT_PROVIDER_ID,
      "providerId",
    );
    ...
    const timeoutMs = this.readPositiveInteger(
      options.timeoutMs ?? WEBHOOK_NOTIFICATION_DEFAULT_TIMEOUT_MS,
      "timeoutMs",
    );
    const backoffBaseMs = this.readNonNegativeInteger(
      options.backoffBaseMs ?? WEBHOOK_NOTIFICATION_DEFAULT_BACKOFF_BASE_MS,
      "backoffBaseMs",
    );
  ...
  private validateRequest(request: NotificationProviderRequest): void {
    if (!request || typeof request !== "object") {
      throw new RuntimeError(
        GovernorErrorCode.NOTIFICATION_DISPATCH_INPUT_INVALID,
        "Webhook notification request must be a non-null object.",
      );
    }
    ...
    if (request.channel !== this.channel) {
      throw new RuntimeError(
        GovernorErrorCode.NOTIFICATION_DISPATCH_INPUT_INVALID,
        `Webhook provider only supports channel "${this.channel}".`,
        {
  ...
  /**
   * Builds deterministic JSON payload sent to the webhook endpoint.
   * @param request Provider request.
