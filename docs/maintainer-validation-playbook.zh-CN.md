...
把 `.tmp/project-113-sprint-005-vscode-distribution-report.json` 视为本轮 rerun 的工作输出即可；在刷新 `docs/support-matrix*` 或任何 maintainer 回链之前，先把 rerun 结果提升为 `.repo-ai-governor/context/dev/project-113-vscode-primary-workbench-full-cutover/sprint-005-phase-h-support-promotion-and-distribution-readiness/` 下带时间戳的 immutable snapshot，并始终让公开证据回链指向这份 snapshot。
...
4. 对 `project-113` 的 VS Code 主工作台边界，权威回链固定为 `.repo-ai-governor/context/dev/project-113-vscode-primary-workbench-full-cutover/sprint-005-phase-h-support-promotion-and-distribution-readiness/` 下的带时间戳 snapshot；`.tmp/project-113-sprint-005-vscode-distribution-report.json` 只保留为 rerun 工作输出。
...
3. `.repo-ai-governor/context/dev/project-113-vscode-primary-workbench-full-cutover/sprint-005-phase-h-support-promotion-and-distribution-readiness/project-113-sprint-005-vscode-distribution-report-20260417T171401Z.json`（当前 VS Code 主工作台声明的权威 snapshot）
