...
将 Claude Code 入口能力模块化到独立 adapter，实现与 `adapter-sdk` 一致的执行契约、能力声明和降级行为。

## 2. 模块边界与职责

1. 目标模块：`packages/adapters/claude-code/`。
2. 模块职责：
   - 实现 `probe/invokeStage/streamEvents/requestConfirmation` 统一接口。
   - 暴露 Claude 能力声明、错误归一与运行时元数据。
3. 依赖约束：
   - 仅依赖 `adapter-sdk/shared-types/shared-utils`。
   - 不依赖 `apps/cli` 与 `core-runtime`。

## 3. Claude Adapter 契约（Draft）
...
1. `TK-404`：消费 Claude 能力声明汇总能力矩阵。
2. `TK-405`：消费 Claude 契约补齐 adapter 契约测试。
3. `TK-406`：消费 Claude 模块边界冻结 CLI 路由层适配策略。

## 7. 验收标准

1. Claude adapter 模块边界与接口语义清晰。
