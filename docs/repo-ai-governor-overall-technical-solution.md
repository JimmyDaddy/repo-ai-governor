...
4. 默认知识源与状态源映射
   ...
   - 执行状态源（Operational State Sources）：`<workspace_root>/context/current-context.md` 与任务台账产物。
...
2. 持久化模式
   - `tool_managed`（默认）：若未配置，数据落在工具侧工作区（建议：`~/.repo-ai-governor/workspaces/<repo_fingerprint>/`）。
   - `repo_local`：用户显式配置后，数据落在目标仓库的 `.repo-ai-governor/`。
3. 解析优先级
...
1. 角色定义入口
   - 通过结构化配置注册自定义角色（建议入口：`governor.yaml -> automation.roles`）。
2. 自定义角色最小字段
   - `role_profile_id`, `display_name`, `responsibilities`, `capabilities`, `permission_ceiling`。
3. 绑定方式
...
3. 角色事实源由 `Role Registry` 统一管理，支持默认角色与用户自定义角色并存。
4. workspace 事实源由 `governor.yaml.workspace` 管理，支持 `tool_managed/repo_local`。
...
3. `skill_id`, `skill_version`（可为空，未加载 skill 时）
4. `status`, `started_at`, `ended_at`, `started_at_display`, `ended_at_display`, `error`
...
5. `execution_session_id`, `memory_scope`, `memory_delta`
6. `notification_channel`, `notification_status`, `notified_at_display`（可选，命中 HITL 通知时记录）
...
本矩阵与 `docs/product-requirements.md` 的 `§10.1` 以及 `docs/repo-ai-governor-architecture-and-repo-layering.md` 的 `§7.1` 保持同步。
...
5. 架构图与仓库分层工程蓝图见 `docs/repo-ai-governor-architecture-and-repo-layering.md`。
