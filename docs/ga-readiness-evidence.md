...
- Scope: `project-026 / sprint-004 / TK-302`, refreshed by `project-046 / sprint-001 / TK-555`
...
|---|---|---|---|
| #1 | At least two pilot repos complete a supported onboarding chain (`install` or supported `dist` binary rehearsal) -> `init -> doctor -> check` within 15 minutes | Pass | `.tmp/project-046-p1-ga-onboarding-timing.json` records normalized per-pilot timing rows: `playground-link=2423ms` and `react-native-image-marker-dist=1136ms`, both under the 15-minute threshold. |
...
2. Refresh `.tmp/project-046-p1-ga-onboarding-timing.json` whenever supported onboarding commands or pilot-entry modes change materially.
