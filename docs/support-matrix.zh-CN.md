...
| VS Code extension（`apps/vscode-extension`） | 已构建源码仓 checkout + extension-development host | editor-native companion MVP 正式支持 | 先构建 governor 源仓，再从 `apps/vscode-extension` 启动扩展开发宿主；所有 trust-sensitive action 仍受 `Workspace Trust` 保护，并继续通过 service-owned query/command seam 消费真值，禁止在 extension 侧制造 execution shadow state。 |
...
| Desktop sidecar entry | 已构建源码仓 / 本地分发演练 | 仅 MVP foundation 正式支持 | `apps/desktop` 继续作为 foundation surface，提供 service-owned session/execution/HITL/artifact-pane seam；更丰富的 desktop 面板仍按后续阶段演进。 |

## 6. 验证快照（TK-301 + TK-547 + TK-551/TK-552/TK-554 + TK-589/TK-590/TK-591 + TK-592/TK-593/TK-594 + TK-598/TK-599/TK-600 + TK-601/TK-602/TK-603 + TK-604/TK-605/TK-606 + TK-607/TK-608/TK-609）

| 时间（UTC） | 命令 | 结果 | 证据摘要 |
|---|---|---|---|
| 2026-04-07T02:18:47Z | `pnpm exec vitest run apps/vscode-extension/test/vscode-extension-contract.test.ts apps/vscode-extension/test/vscode-extension-controller-and-provider.test.ts apps/vscode-extension/test/vscode-extension-presentation-builder.test.ts apps/vscode-extension/test/vscode-extension-selection-store.test.ts apps/vscode-extension/test/vscode-extension-packaging-boundary.test.ts --maxWorkers=1 --maxConcurrency=1` | Pass | VS Code extension 的 contract、controller/provider 流程、presentation mapping、selection-store 生命周期与 packaging-boundary guard 在修复 reviewer finding 后的同一轮定向验证切片中全部通过（`5` 个文件 / `9` 个测试）。 |
...
|---|---|---|---|---|---|---|---|---|---|
| VS Code secondary surface boundary | adopter + maintainer | source-checkout editor-native companion | Pass | 2026-04-07 | `apps/vscode-extension/README.md`、`docs/support-matrix.zh-CN.md`、`docs/local-adoption-playbook.zh-CN.md`、`docs/maintainer-validation-playbook.zh-CN.md`、`pnpm exec vitest run apps/vscode-extension/test/vscode-extension-contract.test.ts apps/vscode-extension/test/vscode-extension-controller-and-provider.test.ts apps/vscode-extension/test/vscode-extension-presentation-builder.test.ts apps/vscode-extension/test/vscode-extension-selection-store.test.ts apps/vscode-extension/test/vscode-extension-packaging-boundary.test.ts --maxWorkers=1 --maxConcurrency=1`、`pnpm pack --json --dry-run`、`pnpm run check:ide-entry-smoke`、`pnpm run check:ide-docs-parity` | 当前支持边界已明确把真实 VS Code extension 提升为首选 secondary surface，但仅限于已构建源码仓的使用路径。packaging-boundary test 与 dry-run package manifest 一起证明：正式支持仍不包含 packaged npm/VSIX 分发，tarball 中即使出现内部 `dist/apps/vscode-extension/**` 也只属于非安装型 runtime payload；`check:ide-docs-parity` 只覆盖受检的 IDE 模板文档面，不直接证明这张公开 support-truth 表本身。 | `docs/local-adoption-playbook.zh-CN.md`、`docs/maintainer-validation-playbook.zh-CN.md`、`apps/vscode-extension/README.md` | extension packaging path、trust-gating contract 或 secondary-surface 叙事变化 | 若后续要支持正式 VSIX 或 packaged delivery，必须新增独立 packaging truth 行并补齐专门演练证据。 |
