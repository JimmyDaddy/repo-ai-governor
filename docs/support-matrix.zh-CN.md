...
| 宿主分发与 service host（`host export/verify/pack`、`@cjhdev/repo-ai-governor/service-host`） | 支持 staged asset generation | 这是位于首选整仓 `adopt apply` 路径之下的低层 staged-asset 能力。典型路径是先用 `host export --mode project-local` 生成宿主原生 repo 资产，需要时再用 `--apply-to-repo` 落到目标仓库，最后通过 `host verify` 校验 manifest/receipt；如果目标支持可安装 bundle，则使用 `host pack --mode plugin-bundle`。当前公开 host family 覆盖 `codex`、`claude-code` 与 `github-copilot`；host 生成依赖仓库本地 `.codex/skills`，而根包的 `service-host` 导出是 clean-room/desktop bootstrap 唯一正式支持的导入路径。 |
...
## 6. 验证快照（TK-301 + TK-547 + TK-551/TK-552/TK-554 + TK-589/TK-590/TK-591 + TK-592/TK-593/TK-594 + TK-598/TK-599/TK-600 + TK-601/TK-602/TK-603 + TK-604/TK-605/TK-606 + TK-607/TK-608/TK-609 + TK-610 + TK-614/TK-615 + TK-616 + TK-656~TK-667）
...
|---|---|---|---|---|---|---|---|---|---|
| adoption-pack installer + self-host bootstrap | adopter + maintainer | 受管 `adopt` 整仓安装故事 | Pass | 2026-04-08T19:37:55Z | `.tmp/project-061-adoption-pack-cleanroom-summary.json` | 内置 `adopt apply` 现已可以在不要求预先存在 source-local `.codex/skills/**` 的前提下，安装受管 multi-host adopter baseline 与 `self-host-complete` repo-local template bootstrap；同一份证据也同时确认了 clean `diff/upgrade/remove` lifecycle 行为，以及当前安装元数据布局位于 `.repo-ai-governor/adoption/installations/**`。 | `README.zh-CN.md`、`docs/local-adoption-playbook.zh-CN.md` | `adopt` contract、managed metadata layout 或 self-host template 语义变化 | multi-host install 目前仍把 receipt/backlink 链聚焦在 primary host apply report 上，但 managed-file verification 仍覆盖完整 projected file set。 |
