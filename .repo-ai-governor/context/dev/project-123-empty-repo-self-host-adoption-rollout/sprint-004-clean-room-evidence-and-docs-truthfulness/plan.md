...
| TK-1064 | refresh self-host docs truth and finalize rollout closeout | run empty-repo self-host clean-room rehearsal and capture rollout evidence | completed |

## 3. Exit Criteria

1. fresh empty-repo clean-room rehearsal、public docs truthfulness、delivery evidence 与 project closeout 已齐备。
2. project-123 closeout 所需 completion audit、delivery registry completed truth 与 idle-context 恢复已全部落盘。
...
7. 2026-05-14：`CR-003` project-final round 已 clean `resolved`，`TK-1064 / DA-1065` 已完成 project-final closeout write-back；当前 sprint 已恢复为最终 `completed` 真值，并从 active execution surface 迁回 completed history。
