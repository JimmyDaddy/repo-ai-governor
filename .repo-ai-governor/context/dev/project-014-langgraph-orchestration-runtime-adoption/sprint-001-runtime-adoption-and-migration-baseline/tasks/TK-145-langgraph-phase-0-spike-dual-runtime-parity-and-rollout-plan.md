...
定义 `LangGraph` Phase 0 spike、`legacy/langgraph` dual-runtime parity、checkpointer 路径与 rollout 迁移计划的正式验收矩阵。
...
1. `DA-145` LangGraph spike、dual-runtime parity 与 rollout 迁移计划。
...
3. `.repo-ai-governor/context/dev/project-014-langgraph-orchestration-runtime-adoption/sprint-001-runtime-adoption-and-migration-baseline/tasks/DA-142-langgraph-runtime-adoption-and-migration-baseline.md`
...
2. 冻结 dual-runtime parity 的比较维度：输出契约、artifact/audit/ledger 一致性、failure semantics、replay/recovery 语义。
3. 确定 checkpointer 路径与 rollout 顺序：file-backed -> sqlite-fs -> shared local orchestration service。
