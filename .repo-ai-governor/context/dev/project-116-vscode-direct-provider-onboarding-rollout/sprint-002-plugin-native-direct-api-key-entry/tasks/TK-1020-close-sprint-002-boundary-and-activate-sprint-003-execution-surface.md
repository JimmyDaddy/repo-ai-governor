# TK-1020 close sprint-002 boundary and activate sprint-003 execution surface
...
After sprint-002 reaches clean `TK/CR` terminal state, close the sprint boundary, activate sprint-003, and prepare the local boundary commit.
...
7. /Users/jimmydaddy/study/ai-governor/.repo-ai-governor/context/dev/project-116-vscode-direct-provider-onboarding-rollout/sprint-002-plugin-native-direct-api-key-entry/tasks/CR-001.md
...
1. 在 `CR-001` resolved 后完成 sprint-002 的 review/task truth、plan、history 与 `current-context.md` closeout write-back。
2. 把 sprint-003 标记为新的 primary execution surface，并让 `TK-1010` 进入 `in_progress`。
3. 运行 sprint-002 boundary gate，准备当前 sprint 的 local boundary commit。
...
1. 2026-04-20：任务创建，用于承接 sprint-002 reviewer-clean 之后的 closeout、boundary gate 与 sprint-003 activation truth 切换。
2. 2026-04-20：`CR-001` 已 resolved；sprint-002 的 review/task truth、project plan、sprint plans、completed history 与 `current-context.md` 已统一切换到 closeout-ready / sprint-003 activation truth。
3. 2026-04-20：`TK-1010` 已在同窗口切换为 `in_progress`，作为 sprint-003 的首个 active execution boundary；下一步只保留 sprint-002 boundary gate 与 local commit。
...
4. `.repo-ai-governor/context/dev/project-116-vscode-direct-provider-onboarding-rollout/sprint-002-plugin-native-direct-api-key-entry/plan.md`
