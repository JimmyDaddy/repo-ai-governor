...
这样做的原因只有一个：避免 `Markdown 表格镜像` 与 `CSV 主注册表` 双写后产生状态漂移。

## Single Source Of Truth

1. `.repo-ai-governor/context/artifact-registry/artifacts.csv` 是主注册表唯一事实源。
2. `.repo-ai-governor/context/artifact-registry/archive/artifacts.archive.csv` 是归档注册表唯一事实源。
3. 本文件只保留规则、命令与检索方式，不再记录 artifact 行。
4. 任何人类可读视图都应从 CSV 动态渲染，而不是在 Markdown 中重复维护。
...
5. 主注册表仅保留 `active/frozen/deprecated`；`archived/retired` 必须迁移到归档注册表。
