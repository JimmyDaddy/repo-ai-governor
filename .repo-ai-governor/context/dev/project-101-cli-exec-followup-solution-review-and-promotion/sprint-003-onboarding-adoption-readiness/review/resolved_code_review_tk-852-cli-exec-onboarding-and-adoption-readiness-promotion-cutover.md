# Code Review: TK-852 cli-exec onboarding and adoption readiness promotion cutover
...
4. `.repo-ai-governor/normative_knowledge_sources/technical-solutions/runtime-agent-projection/adrs/cli-exec-onboarding-and-adoption-readiness-productization.md`
...
9. `.repo-ai-governor/context/dev/project-104-cli-exec-onboarding-adoption-readiness-rollout/**`
...
1. 本轮 formalize 的是 readiness evidence chain 的 runtime guidance、ownership split 与 rollout boundary，不是新的 minimum contract 或 public support truth。
2. lifecycle `final_paths` 仅占有 solution-owned ADR；overview 与两份 contract 继续作为 shared formal docs 复用。
3. delivery handoff 已指向新的 `project-104` planned rollout skeleton，符合 `followup_required + adopter_cli + docs_playbook + planned rollout` 的治理边界。
