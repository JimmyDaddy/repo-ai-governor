...
2. 将 role、surface、session、capability、budget 与 timeout 组合成 agent descriptor 视图。
3. 为 CLI、report、diagnostics 与后续 UI 提供同一份 agent projection 数据。
4. 将 `AgentSessionRegistry` 作为共享 session 的投影层，而不是新的会话事实源。
5. 允许 LangGraph supervisor 消费 agent descriptor，但不把 supervisor 升格为新的 canonical runtime。
