...
7. 后续真实实现与 clean-room rollout 由 `project-061-adoption-pack-installer-and-self-host-bootstrap-rollout` 承接。
...
4. layered resolution 保留了 repo-local override 的扩展能力，同时让官方内置 pack 可以成为默认 adopter story。
...
3. `self-host-complete` 成为正式高级 profile，但其落地仍以 repo-local template bootstrap 为边界，不宣称实现面已经完成。
4. delivery handoff 切换到新的 planned follow-up stream `project-061-adoption-pack-installer-and-self-host-bootstrap-rollout`。

## 5. Follow-Up

1. `project-061` sprint-001：manifest / installer contract freeze 与 layered resolver baseline
2. `project-061` sprint-002：`adopt apply`、managed ownership 与 install receipt
3. `project-061` sprint-003：`adopter-complete` content coverage 与 host materialization
4. `project-061` sprint-004：`diff/upgrade/remove` 与 adoption verify
5. `project-061` sprint-005：`self-host-complete` template bootstrap 与 governance authoring surface
6. `project-061` sprint-006：clean-room rehearsal、README/playbook/support matrix truthfulness closeout
