...
- [CS-021] Task and sprint ledgers must stay synchronized: for each active stream, canonical `TK/CR` task cards, `tasks/checklist.md`, and canonical rows in `tasks/tasks.csv` must stay synchronized for `title/status/owner/priority/project/sprint/plan/recorded_at`; and for all sprint directories under `.repo-ai-governor/context/dev/**`, sprint `plan.md` status must align with latest `tasks/tasks.csv` aggregate status (`planned/active/completed`). `TK` 终态为 `completed`，`CR` 终态为 `resolved`；当两类任务均进入终态时，不得继续让 sprint 保持 active 而不进入 closeout。
...
- [CS-023] Artifact Registry must enforce lifecycle exit governance: `artifact_status` only uses `active/frozen/deprecated/archived/retired`; machine-readable canonical registry defaults to `.repo-ai-governor/context/artifact-registry/sqlite/artifact-registry.sqlite`; rendered main view (`.repo-ai-governor/context/artifact-registry/artifacts.csv`) only keeps `active/frozen/deprecated`; rendered archive view (`.repo-ai-governor/context/artifact-registry/archive/artifacts.archive.csv`) only keeps `archived/retired`; stale `deprecated` entries beyond grace window must be archived; rendered CSV views must stay rebuildable from canonical truth and must not be hand-maintained as an independent source.
...
- [CS-032] Avoid scattered magic literals. Numeric, string, and boolean literals that carry domain/business/configuration semantics must be extracted into named constants (under `src/constants` or package-level `packages/*/src/constants`) or enum members. Exceptions that do not require extraction: (1) structurally obvious values such as `0`, `1`, `''`, `true`, `false` in arithmetic, guard clauses, or initializers where meaning is self-evident from context; (2) test assertion expected values; (3) log/error message template strings. Occurrences outside exceptions require a nearby marker comment `// literal-allowed: reason` when extraction is intentionally deferred.
- [CS-033] All user-facing text in `apps/**` and `packages/**` must go through the project i18n system (`I18nRuntime.t()` for keyed translations, or the `localizeText(english, chinese)` bridge for inline bilingual copy). Hardcoded single-language strings in CLI prompts, error messages shown to users, help descriptions, and interactive shell labels are forbidden. New user-facing keys must be added to both `packages/shared/src/i18n/locales/en-us.ts` and `packages/shared/src/i18n/locales/zh-cn.ts` and must pass the `check-i18n-parity-fallback` gate. Exceptions: (1) internal log messages not surfaced to end users; (2) structured output field names and machine-readable identifiers; (3) test fixture strings. Deferred cases require a nearby marker comment `// i18n-deferred: reason`.
- [CS-034] Completion/build evidence is mandatory for code-affecting changes. When a change window modifies executable or typed surfaces under `apps/**`, `packages/**`, `bin/**`, or `test/**`, any claim that the work is `completed`, `resolved`, `done`, or “all green” must be backed by one real `pnpm run build` executed in the same change window and recorded in verification evidence. Targeted tests and partial gates do not substitute for build evidence. Docs-only or ledger-only changes may skip build only when the closeout explicitly states that no executable code changed.
