...
| 当前外部 adoption/productization 收口主线 | `project-020-adoption-productization-and-upgrade-ux` | active | `sprint-002-packaged-runtime-cutover-and-release-gate-block` 已完成并确认 packaged docs/skills/offline-install truthfulness 收口；当前 worktree 仅临时保留其 closeout surface，下一条推荐执行流为 `sprint-003` |

## 3.4 当前推荐执行顺序

1. `project-020 / sprint-003-upgrade-and-workspace-lifecycle-ux-baseline` 为当前推荐的下一条执行流，用于把已有 upgrade/workspace 服务层能力变成 adopter 可操作的 CLI 用户路径。
2. `project-020 / sprint-004-adopter-pilot-and-documentation-closure` 为已拆解但未激活的 follow-up sprint，用真实目标仓库验证接入、升级、回滚与 support matrix。
3. `project-020 / sprint-002-packaged-runtime-cutover-and-release-gate-block` 已完成，但当前 worktree 仍临时保留其 closeout surface。
4. `project-020 / sprint-001-packaging-truthfulness-failure-baseline` 已完成并迁入 completed history，作为 sprint-002 的 baseline 证据面保留。
...
| `project-020-adoption-productization-and-upgrade-ux` | Post-Stage-9 external adoption closure | active | 已完成 sprint-002 packaged cutover 与 blocking gate 收口；下一步应显式激活 sprint-003，把 upgrade/workspace 能力收敛为 adopter CLI 用户路径 |
