...
2. 当前未完成的部分：
   - 当前主线未完成项已经切换到 `project-021 / sprint-002`：需要在 `runtime.memory-semantics` 现有 recall/context assembly baseline 之上，引入显式 promotion pipeline，并让第二 runtime consumer 通过 contract-safe summary 接入。
...
| 当前 runtime semantics 实现主线 | `project-021-memory-semantics-runtime-implementation` | active | `sprint-001` 已完成 recall/context assembly baseline，当前已激活 `sprint-002-promotion-pipeline-and-runtime-consumer-rollout` |
...
1. `project-021 / sprint-002-promotion-pipeline-and-runtime-consumer-rollout` 已激活，是当前默认主执行流；优先完成 memory promotion pipeline baseline 与第二 runtime consumer rollout。
...
| `project-021-memory-semantics-runtime-implementation` | Post-Stage-9 runtime semantics implementation | active | 已激活 `sprint-001-recall-context-assembly-baseline`，负责将 `runtime.memory-semantics` formal solution 转成运行时代码，并通过 delivery handoff 机制承接 `technical-solution.memory-module` |
