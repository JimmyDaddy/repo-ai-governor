  ...
  return sectionBody
    ...
    .map((entry, index) => {
      ...
      const targetLine = lines.find(
        (line) =>
          line.startsWith("Target:") || line.startsWith("目标:") || line.startsWith("目标："),
      );
      ...
      const suggestionLine = lines.find(
        (line) =>
          line.startsWith("Suggestion:") || line.startsWith("建议:") || line.startsWith("建议："),
      );
      ...
      return {
        ...
        status:
          headerMatch?.[1] === "error" ? "fail" : headerMatch?.[1] === "warning" ? "warn" : "pass",
        message: headerMatch?.[3] ?? lines[0],
        target: targetLine
          ? stripBackticks(targetLine.replace(/^(Target|目标)[:：]\s+/, "").trim())
          : null,
        ruleId: ruleLine
          ? stripBackticks(ruleLine.replace(/^(Rule|规则)[:：]\s+/, "").trim())
          : null,
        suggestion: suggestionLine
          ? suggestionLine.replace(/^(Suggestion|建议)[:：]\s+/, "").trim()
          : null,
      };
    });
}
...
function parseSummary(
  ...
  const countsLine = content.match(
    /(?:Errors|错误):\s+`(\d+)`(?:,|，)\s*(?:Warnings|warnings|告警):\s+`(\d+)`/,
  );
...
export function loadReportSource(
  ...
  if (!fs.existsSync(absoluteSourcePath)) {
    throw new InputError(
      t(
        locale,
        `未找到报告来源文件：${absoluteSourcePath}`,
        `Report source file not found: ${absoluteSourcePath}`,
      ),
      {
  ...
  throw new InputError(
    t(
      locale,
      `不支持的报告来源格式：${absoluteSourcePath}`,
      `Unsupported report source format: ${absoluteSourcePath}`,
    ),
    {
