...
function createRule(rule: StandardsRule): StandardsRule {
...
export const OFFICIAL_BASE_PACKAGE_RULES = Object.freeze([
  createRule({
    ...
    title: createLocalizedText(
      "代码改动应遵循现有仓库结构",
      "Code Changes Should Follow Existing Repository Structure",
    ),
  ...
  createRule({
    ...
    level: "required",
    title: createLocalizedText(
      "交付必须使用 Conventional Commit",
      "Delivery Must Use Conventional Commits",
    ),
    statement: createLocalizedText(
  ...
  createRule({
    ...
    title: createLocalizedText(
      "方案必须明确目标、范围与风险",
      "Plans Must State Goals, Scope, and Risks",
    ),
    statement: createLocalizedText(
  ...
  createRule({
    ...
    level: "required",
    title: createLocalizedText(
      "任务拆解必须同步 checklist 与 CSV",
      "Task Breakdown Must Sync Checklist and CSV",
    ),
    statement: createLocalizedText(
  ...
  createRule({
    ...
    title: createLocalizedText(
      "交付前必须给出验证路径",
      "Delivery Must Include a Verification Path",
    ),
    statement: createLocalizedText(
  ...
  createRule({
    ...
    title: createLocalizedText(
      "校验结果必须记录到任务台账",
      "Check Results Must Be Recorded in Task Records",
    ),
    statement: createLocalizedText(
...
export function resolveStandardsPackage(standardsConfig: StandardsConfig = {}): StandardsPackage {
  ...
  if (
    Array.isArray(standardsConfig.locales?.supported) &&
    standardsConfig.locales.supported.length > 0
  ) {
...
export function listRulesForConsumer(
...
export function renderRulesForConsumer(
  standardsPackage: StandardsPackage,
  consumer: StandardsConsumer,
