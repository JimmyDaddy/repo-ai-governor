...
function buildAdapterTemplate(adapterId: string, locale: string): AnyRecord {
  return {
    id: adapterId,
    version: "1",
    type: "ide-or-cli",
...
function sortObjectKeys(value: AnyRecord): AnyRecord {
  return Object.fromEntries(
    Object.entries(value ?? {}).sort(([leftKey], [rightKey]) => leftKey.localeCompare(rightKey)),
  );
}

function resolvePackageManager(cwd: string): { name: string; installCommand: string[] } {
...
function ensureLocalPackageManifest(
  cwd: string,
  ...
  const previousSerializedContent = packageFileExists
...
function installBundledSkills(options: AnyRecord): AnyRecord {
  const cwd = String(options.cwd ?? process.cwd());
  const surfaces = Array.isArray(options.surfaces) ? (options.surfaces as string[]) : [];
  ...
  const normalizedSurfaces = [
    ...new Set(surfaces.map((surface: string) => validateSkillSurface(surface))),
  ];
  ...
  return {
    ...
    enabledAdaptersMarkdown:
      config.adapters.enabled.length > 0
        ? config.adapters.enabled.map((adapterId: string) => `\`${adapterId}\``).join(", ")
        : "`none`",
    csvColumns: config.artifacts.taskFiles.csvColumns,
  };
}

function resolveSprintArtifacts(cwd: string, config: AnyRecord): AnyRecord {
...
function applyInitDefaults(config: AnyRecord, cwd: string): AnyRecord {
...
function applyConfigRootOverrides(
  config: AnyRecord,
  cwd: string,
...
function buildInitPlan(commandContext: AnyRecord): AnyRecord {
  ...
  const files = [
    ...
    },
    {
      ...
      action: fs.existsSync(path.resolve(cwd, config.slots.directory, "README.md"))
        ? "update"
        : "create",
    },
    {
      ...
      action: fs.existsSync(
        path.resolve(path.dirname(resolved.paths.configFile), "templates", "README.md"),
      )
        ? "update"
        : "create",
    },
...
function detectConflicts(files: AnyRecord[]): string[] {
  return files
    .filter((file: AnyRecord) => fs.existsSync(file.path))
    .map((file: AnyRecord) => file.path);
}

function renderInitPayload(
  plan: AnyRecord,
  commandContext: AnyRecord,
  options: AnyRecord = {},
): AnyRecord {
  return {
    command: "init",
    ...
    directories: plan.directories.map((directoryPath: string) =>
      toRelativePath(plan.cwd, directoryPath),
    ),
    files: plan.files.map((file: AnyRecord) => ({
...
function writeInitSummary(logger: AnyRecord, payload: AnyRecord, format: string): void {
  ...
  logger.keyValue(
    t(locale, "入口文件", "Agent entry"),
    toRelativePath(payload.cwd, payload.agentEntry),
  );
  logger.keyValue(
    t(locale, "上下文文件", "Context file"),
    toRelativePath(payload.cwd, payload.contextFile),
  );
  logger.keyValue(t(locale, "项目", "Project"), payload.currentProject);
  logger.keyValue(t(locale, "Sprint", "Sprint"), payload.currentSprint);
  logger.keyValue(
    t(locale, "已启用适配器", "Enabled adapters"),
    JSON.stringify(payload.enabledAdapters),
  );
  ...
  if (payload.skillBootstrap) {
    logger.keyValue(
      t(locale, "Skill 安装", "Skill bootstrap"),
      JSON.stringify(payload.skillBootstrap),
    );
  }
...
export function executeInitCommand(commandContext: AnyRecord, logger: AnyRecord): void {
  ...
  const shouldSelfInstall =
    !dryRun && !skipSelfInstall && (selfInstallRequested || isNpxInvocation());
  const shouldInstallSkills = !dryRun && !skipSkillInstall;
  const dependencyBootstrap: AnyRecord = {
  ...
  if (!dryRun) {
    ...
    if (shouldInstallSkills) {
      ...
      skillBootstrap = {
        ...
        targetRoots: installState.targetRoots.map((targetRoot: string) =>
          toRelativePath(plan.cwd, targetRoot),
        ),
        operations: installState.operations.map((operation: AnyRecord) => ({
          ...operation,
          path: toRelativePath(plan.cwd, operation.path),
        })),
        summary: {
          installed: installState.operations.filter(
            (operation: AnyRecord) => operation.status === "installed",
          ).length,
          skipped: installState.operations.filter(
            (operation: AnyRecord) => operation.status === "skipped",
          ).length,
        },
      };
    }
