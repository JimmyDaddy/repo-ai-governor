...
const CHECK_WORKFLOW_TEMPLATE = Object.freeze({
  ...
  meta: {
    ...
    description: {
      ...
      "en-US":
        "Validates planning, breakdown, and execution records against the minimum governance requirements.",
    },
  },
  execution: {
    mode: "serial",
  ...
  stages: [
    ...
    },
    {
      ...
      description: {
        ...
        "en-US":
          "Validates verification records, verify fields, and the minimum task ledger quality signals.",
      },
      dependsOn: ["breakdown"],
      executor: {
        kind: "internal",
...
function buildArtifactPaths(cwd: string, resolvedConfig: AnyRecord, locale = "zh-CN"): AnyRecord {
  ...
  if (!currentProject || !currentSprint) {
    throw new ConfigError(
      t(
        locale,
        "check 命令需要当前 project 与 sprint。",
        "Check command requires a current project and sprint",
      ),
      {
...
function buildCheckRun(commandContext: CommandContext): AnyRecord {
  ...
  if (!CHECK_WORKFLOW_TEMPLATE.stages.some((stage) => stage.id === selectedStage)) {
    throw new InputError(
      t(
        locale,
        `不支持的 check 阶段：${selectedStage}`,
        `Unsupported check stage: ${selectedStage}`,
      ),
      {
  ...
  const requiredSections = [
    {
      ...
      missingMessage: t(
        locale,
        "plan 缺少必需章节：goal。",
        "Plan is missing the required goal section.",
      ),
      suggestion: t(
        locale,
        "请在 plan.md 中补充 Goal/目标 章节。",
        "Add a Goal section to plan.md.",
      ),
    },
    {
      sectionId: "inScope",
      ...
      missingMessage: t(
        locale,
        "plan 缺少必需章节：inScope。",
        "Plan is missing the required inScope section.",
      ),
      suggestion: t(
        locale,
        "请在 plan.md 中补充 In Scope/纳入范围 章节。",
        "Add an In Scope section to plan.md.",
      ),
    },
    {
      ...
      missingMessage: t(
        locale,
        "plan 缺少必需章节：outOfScope。",
        "Plan is missing the required outOfScope section.",
      ),
      suggestion: t(
        locale,
        "请在 plan.md 中补充 Out Of Scope/非范围 章节。",
        "Add an Out Of Scope section to plan.md.",
      ),
    },
    {
      sectionId: "risks",
      ...
      missingMessage: t(
        locale,
        "plan 缺少必需章节：risks。",
        "Plan is missing the required risks section.",
      ),
      suggestion: t(
        locale,
        "请在 plan.md 中补充 Risks/风险 章节。",
        "Add a Risks section to plan.md.",
      ),
    },
    {
      ...
      missingMessage: t(
        locale,
        "plan 缺少必需章节：acceptance。",
        "Plan is missing the required acceptance section.",
      ),
      suggestion: t(
        locale,
        "请在 plan.md 中补充 Acceptance/验收标准 章节。",
        "Add an Acceptance section to plan.md.",
      ),
    },
    {
      ...
      passMessage: t(
        locale,
        "plan 已包含验证路径章节。",
        "Plan contains a verification path section.",
      ),
      missingMessage: t(
        locale,
    ...
    findings.push(
      createFinding({
        ...
        message: t(
          locale,
          "在 tasks/ 下未找到任务卡文件。",
          "No task card files were found under tasks/.",
        ),
    ...
    findings.push(
      createFinding({
        ...
        message: t(
          locale,
          "checklist 已包含执行记录。",
          "Checklist contains execution log records.",
        ),
        target: checklistTarget,
      }),
    );
    ...
    findings.push(
      createFinding({
        ...
        message: t(
          locale,
          "checklist 未包含执行记录。",
          "Checklist does not contain execution log records.",
        ),
        target: checklistTarget,
        suggestion: t(
          locale,
  ...
  if (csvLines.length > 1) {
    ...
    findings.push(
      createFinding({
        ...
        message: t(
          locale,
          "tasks.csv 尚未包含任何执行记录行。",
          "tasks.csv does not contain any execution rows.",
        ),
        target: csvTarget,
        suggestion: t(
          locale,
  ...
  if (workflowResult.status === "failed" && workflowResult.failure) {
    findings.push(
      createFinding({
        ...
        suggestion: t(
          locale,
          "请先修复该阶段失败原因后再重新执行 check。",
          "Resolve the stage-level workflow failure before rerunning check.",
        ),
      }),
    );
  }
...
function writeCheckSummary(logger: Logger, payload: AnyRecord, format: string): void {
  ...
  logger.keyValue(
    t(locale, "命中规则", "Matched rules"),
    JSON.stringify(payload.standards.matchedRuleIds),
  );
...
function writeReportFile(runState: AnyRecord, payload: AnyRecord, format: string): string {
  ...
  const reportFileName =
    runState.resolvedConfig.config.reporting.fileNames?.[format] ??
