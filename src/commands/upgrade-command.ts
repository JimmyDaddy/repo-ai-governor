...
function parseGovernorDocument(
  filePath: string,
  locale = "zh-CN",
): {
  rawContent: string;
  document: GovernorDocument;
} {
  try {
    const rawContent = fs.readFileSync(filePath, "utf8");
    const parsed = YAML.parse(rawContent) as unknown;
    const document: GovernorDocument =
      parsed !== null && typeof parsed === "object" && !Array.isArray(parsed)
        ? (parsed as GovernorDocument)
        : {};
    return {
      rawContent,
      document,
    ...
    throw new ConfigError(
      t(
        locale,
        `读取升级来源配置失败：${filePath}`,
        `Failed to read upgrade source config: ${filePath}`,
      ),
      {
...
function buildBackupDirectory(cwd: string, timestamp: string): string {
...
function buildUpgradePlan(commandContext: CommandContext): UpgradePlan {
  ...
  const targetVersion = getStringOption(commandContext.commandOptions, "toVersion") ?? "1";
  ...
  const preview =
    commandContext.commandOptions.preview === true || commandContext.globalOptions.dryRun === true;
  const backup = commandContext.commandOptions.backup === true;
  const backupDir = backup
    ? buildBackupDirectory(cwd, new Date().toISOString().replace(/[:.]/g, "-"))
    : null;
...
function writeBackupFiles(plan: UpgradePlan): string[] {
  if (!plan.backupDir) {
    return [];
  }
...
function writeUpgradeFiles(plan: UpgradePlan): void {
  for (const file of plan.upgradeFiles) {
...
function createUpgradePayload(plan: UpgradePlan, backupTargets: string[]): UpgradePayload {
  return {
    command: "upgrade",
...
function writeUpgradeSummary(logger: Logger, payload: UpgradePayload, format: string): void {
...
export async function executeUpgradeCommand(
