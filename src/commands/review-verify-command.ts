...
const REVIEW_VERIFY_WORKFLOW_TEMPLATE = {
  ...
  meta: {
    ...
    description: {
      ...
      "en-US":
        "Re-validates review findings, appends verification logs, and advances the CR lifecycle.",
    },
  },
  execution: {
    mode: "serial",
...
function extractReviewSlugFromFileName(fileName: string): string {
  return fileName
...
function parseNumberedSectionItems(content: string, headings: string | string[]): string[] {
  ...
  return body
    .split(/\n(?=\d+\.\s)/)
    .map((entry: string) => entry.replace(/^\d+\.\s*/, "").trim())
    .filter(Boolean);
}

function extractTargetsFromSource(cwd: string, sourceContent: string): string[] {
  const targetsSection = extractSectionBody(sourceContent, ["Targets", "目标文件"]);
  const matches = Array.from(
    targetsSection.matchAll(/`([^`]+)`/g),
...
function formatNumberedList(items: string[], locale: string): string {
  if (items.length === 0) {
...
function renderStandardsList(
  ...
  if (standards.length === 0) {
    return t(
      locale,
      "1. 未加载面向 review-verify 阶段的规范。",
      "1. No review-verify standards were loaded.",
    );
  }
...
function buildVerifyEntries(
  ...
  if (analysis.findings.length === 0) {
    entries.push(
      t(runState.locale, "未发现剩余评审问题。", "No remaining review findings were detected."),
    );
  } else {
    entries.push(
      t(
  ...
  if (summary.status === "pass") {
    return [
      t(
        runState.locale,
        "复核前无需额外修复。",
        "No additional fixes were required before verification.",
      ),
    ];
  }
...
function buildMarkdownOutput(payload: ReviewVerifyPayload): string {
  ...
  return ensureTrailingNewline(
    [
      ...
      payload.pathOption
        ? `${t(locale, "路径覆盖", "Path override")}: \`${payload.pathOption}\``
        : "",
      ...
      "",
      formatNumberedList(
        payload.targets.map((target: string) => `\`${target}\``),
        locale,
      ),
      "",
      `## ${t(locale, "摘要", "Summary")}`,
...
function buildReviewVerifyRun(commandContext: CommandContext): ReviewVerifyRunState {
  ...
  if (!fs.existsSync(sourceFilePath)) {
    throw new InputError(
      t(
        requestedLocale,
        `未找到评审来源文件：${sourceFilePath}`,
        `Review source file not found: ${sourceFilePath}`,
      ),
      {
  ...
  return {
    ...
    previousVerifyEntries: parseNumberedSectionItems(sourceContent, [
      ...
      (entry) =>
        entry !==
          "Pending verification. Append review-verify results to this file and rename it to the next review status." &&
    ...
    previousResolutionEntries: parseNumberedSectionItems(sourceContent, [
      ...
      (entry) =>
        entry !== "No resolutions have been applied yet." && entry !== "尚未应用任何解决动作。",
    ),
  };
}

async function executeReviewVerifyWorkflow(runState: ReviewVerifyRunState): Promise<{
  workflowResult: ReviewVerifyWorkflowResult;
  analysis: ReviewAnalysis;
  summary: ReviewSummary;
}> {
  const workflowResult = await executeWorkflow({
    template: REVIEW_VERIFY_WORKFLOW_TEMPLATE as unknown as ExecuteWorkflowOptions["template"],
  ...
  return {
    workflowResult: workflowResult as unknown as ReviewVerifyWorkflowResult,
    analysis: (verifyStage?.outputs.analysis as ReviewAnalysis | undefined) ?? {
      findings: [],
      matchedRuleIds: [],
      relativeTargets: [],
    },
    summary:
      (verifyStage?.outputs.summary as ReviewSummary | undefined) ??
      (summarizeFindings([], {
        failOnWarnings: runState.strict,
      }) as ReviewSummary),
  };
...
function buildReviewVerifyPayload(
  runState: ReviewVerifyRunState,
  workflowResult: ReviewVerifyWorkflowResult,
  analysis: ReviewAnalysis,
  summary: ReviewSummary,
  ...
  const reviewVerifyRules = renderRulesForConsumer(runState.standardsPackage, "review-verify", {
...
function writeVerifiedReviewFile(
  runState: ReviewVerifyRunState,
  payload: ReviewVerifyPayload,
): string {
...
function writeReviewVerifyOutput(
  logger: Logger,
  ...
  if (payload.status === "fail") {
    logger.error(
      t(payload.locale, "复核发现阻断问题", "Review verification found blocking issues"),
    );
  } else if (payload.status === "warn") {
    logger.warn(
      t(payload.locale, "复核发现非阻断问题", "Review verification found non-blocking issues"),
    );
  } else {
  ...
  logger.keyValue(
    t(payload.locale, "生命周期", "Lifecycle"),
    `${payload.reviewStatusBefore} -> ${payload.reviewStatusAfter}`,
  );
...
export async function executeReviewVerifyCommand(
  commandContext: CommandContext,
  logger: Logger,
): Promise<number> {
