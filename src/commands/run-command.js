...
function inferActionFromRouteKey(routeKey) {
  const explicit = ROUTE_ACTION_RULES[routeKey];
...
function inferIntentActionsFromInput(content) {
  const normalizedContent = String(content ?? "");
  const actions = [];
...
function resolveRequiredActions(routeDecisions, policyInputContent) {
...
function resolvePermissionTier(permissions) {
...
function evaluatePolicyGate(runState, routeDecisions) {
  ...
  for (const action of requiredActions) {
    const permissionField = ACTION_PERMISSION_FIELD[action];
  ...
  for (const tag of riskTags) {
    ...
    const permissionField = HIGH_RISK_PERMISSION_FIELD[tag];
...
function createSurfaceSuggestion(surface, checkId, locale) {
...
function runProbe(binary, args, cwd) {
  try {
    ...
    if (result.error) {
      return {
        ok: false,
        code: result.error.code === "ENOENT" ? "probe.binary_missing" : "probe.spawn_error",
...
function resolveSurfaceProbeDefinition(surface, runState) {
  ...
  return BUILTIN_SURFACE_PROBES[surface] ?? null;
}

function runSurfacePreflight(surface, runState) {
...
function buildRunArtifactPaths(cwd, resolvedConfig, locale) {
...
function loadTaskLedger(artifactPaths) {
...
function loadTaskCardIds(tasksRoot) {
...
function buildReviewStatusIndex(codeReviewRoot) {
  const index = new Map();
  ...
  for (const fileName of fs.readdirSync(codeReviewRoot)) {
    ...
    const weight = REVIEW_STATUS_WEIGHT[prefix] ?? 0;
...
function buildTaskBreakdownContext(runState) {
...
function resolveTaskCompletion(task, reviewIndex, completionStatuses) {
  if (completionStatuses.has(task.status)) {
    return {
      complete: true,
      source: "tasks.csv"
...
function resolveRoutingProfile(config, profileId, locale) {
  if (profileId) {
    ...
    const builtinProfile = BUILTIN_ROUTING_PROFILES[profileId];
...
function resolveLoopCompletionPolicy(rawPolicy, locale, stageId) {
...
function resolveReviewLoops(runState, processConfig, stages) {
  ...
  for (const { defaults, config } of loopConfigs) {
    ...
    const routeSequence =
      Array.isArray(config.routeSequence) && config.routeSequence.length > 0
        ? config.routeSequence.map((routeKey) => String(routeKey).trim()).filter(Boolean)
...
function resolveProcessModel(runState) {
  ...
  const reviewLoopStageIds = new Set(reviewLoops.map((loop) => loop.stageId));
...
function parseRouteOverrides(rawRouteValues, processModel, locale) {
...
function buildRoutingPlan(runState, processModel) {
...
function evaluateRoutingDecisions(routingPlan, reportsBySurface, runState) {
  const decisions = [];
  const blocking = [];
  const pausing = [];
  const warnings = [];
    ...
    if (requiredSurfaceStrict && runState.nonInteractive !== true) {
      ...
      pausing.push({
        ...
        suggestion: requestedReport?.checks
          .filter((check) => check.status === "fail" && check.suggestion)
...
function executePreflight(runState, routingPlan) {
  const surfaces = new Set([routingPlan.defaultSurface, ...routingPlan.routes.map((route) => route.requestedSurface)]);
...
function buildRunWorkflowTemplate(processModel) {
  return {
    id: "automation-run",
    version: "1",
    ...
    stages: [
      ...
      ...processModel.stages.map((stage, index) => ({
        id: stage.id,
        name: stage.name,
...
function createDispatchResult(runState, routeDecision, options = {}) {
...
function createRunHandlers(runState, processModel, routingPlan) {
  const routeDecisionMap = new Map(routingPlan.routes.map((route) => [route.routeKey, cloneValue(route)]));

  function getRouteDecision(routeKey) {
  ...
  function handlePreflightStage() {
    if (!runState.preflightEnabled) {
      const skippedPreflight = {
        ...
        status: "skipped",
        reports: [],
        decisions: routingPlan.routes.map((route) => ({
          ...cloneValue(route),
  ...
  function resolveReviewLoopCompletion(loopConfig, cycle) {
    if (runState.dryRun) {
      return {
        complete: true,
        source: "dry-run"
  ...
  function handleReviewLoopStage(loopConfig) {
    const cycles = [];
    let resolved = false;
    let resolvedBy = null;
  ...
  function handleTaskBreakdownStage(stageContext) {
    const stage = processModel.stages.find((candidate) => candidate.id === "task-breakdown");
    const dispatchResult = stage?.routeKey
  ...
  function handleTaskLoopStage(stageContext) {
...
function flattenWorkflowWarnings(workflowResult) {
  return workflowResult.stages.flatMap((stageResult) => stageResult.warnings ?? []);
}

function buildRunSummary(workflowResult, preflightStatus) {
  const stageWarnings = flattenWorkflowWarnings(workflowResult);
  const failedStageCount = workflowResult.stages.filter((stageResult) => stageResult.status === "failed").length;
  const blockedStageCount = workflowResult.stages.filter((stageResult) => stageResult.status === "blocked").length;
  ...
  return {
    ...
    errors,
    warnings,
    passed: workflowResult.stages.filter((stageResult) => stageResult.status === "passed").length,
    exitCode
  };
}
...
function buildRunPayload(runState, processModel, routingPlan, workflowResult, summary) {
  ...
  const policyStage = workflowResult.stages.find((stage) => stage.id === "policy-gate");
...
function writeRunSummary(logger, payload, format) {
...
function buildRunState(commandContext) {
  ...
  const preflightEnabled =
    commandContext.commandOptions.skipPreflight === true
      ? false
      : (resolvedConfig.config.automation.preflight?.enabled ?? true);
  const failOnMissingRequiredSurface =
    resolvedConfig.config.automation.preflight?.failOnMissingRequiredSurface ?? true;
  ...
  const taskCompletionStatuses = Array.isArray(
    resolvedConfig.config.automation.taskLoop?.completionStatuses
  )
    ? resolvedConfig.config.automation.taskLoop.completionStatuses.map(normalizeTaskStatus)
  ...
  const automationPermissions = {
    ...(resolvedConfig.config.automation.permissions ?? {})
  };
  const requiredApprovalTags = new Set(
  ...
  return {
    ...
    routingProfile:
      ...
      resolvedConfig.config.automation.routingProfile ??
      "multi-ai-dev-review",
    ...
    surfaceDefinitions: resolvedConfig.config.automation.surfaces ?? {},
    resolvedConfig,
    artifactPaths,
...
export async function executeRunCommand(commandContext, logger) {
  const runState = buildRunState(commandContext);
