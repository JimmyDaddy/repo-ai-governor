...
function createTaskId(prefix, taskNumber) {
  return `${prefix}-${String(taskNumber).padStart(3, "0")}`;
}

function collectTaskNumbersFromText(content, prefix) {
...
function collectExistingTaskNumbers(paths, prefix) {
  const numbers = [];
...
function createLocalizedTaskBlueprints(locale, title, standardsPackage, startTaskNumber, dateStamp, taskPrefix) {
...
function createPlanStrategy(locale, title) {
  return locale === "en-US"
    ? [
...
function resolveArtifactPaths(cwd, config, commandContext, locale) {
  ...
  const bundleRoot = commandContext.commandOptions.bundleDir
    ? path.resolve(cwd, commandContext.commandOptions.bundleDir)
  ...
  return {
    ...
    outputFile: commandContext.commandOptions.out
      ? path.resolve(cwd, commandContext.commandOptions.out)
      : null,
...
function buildPlanRun(commandContext) {
...
function buildGeneratedFiles(runState, workflowResult) {
  ...
  const files = [
    ...
    ...runState.planningTasks.map((task) => {
...
function writeSummaryOutput(filePath, payload, format) {
  if (!filePath) {
    return null;
  }
...
function createWorkflowHandlers(runState) {
  return {
    plan: ({ state, selectedStageIds }) => {
...
function renderPlanPayload(runState, workflowResult, commandContext, files, outputFilePath = null) {
  return {
    command: "plan",
    ...
    standards: {
      ...
      planRuleIds: listRulesForConsumer(runState.standardsPackage, "plan").map((rule) => rule.id)
    },
    tasks: runState.planningTasks,
...
export async function executePlanCommand(commandContext, logger) {
  let runState;
  ...
  const workflowConfig = structuredClone(runState.resolvedConfig.config.workflow);

  if (commandContext.commandOptions.template) {
    workflowConfig.template = commandContext.commandOptions.template;
  }

  const workflowResult = await executeWorkflow({
    workflowConfig,
    targetStages: ["breakdown"],
    handlers: createWorkflowHandlers(runState),
    initialState: {
      documents: {},
      tasks: []
    },
