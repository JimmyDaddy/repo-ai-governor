...
export function validateWorkflowTemplate(template) {
  const candidate = validateSchemaDocument("workflowTemplate", template);
...
export const STANDARD_WORKFLOW_TEMPLATE = Object.freeze(
    ...
    stages: [
      {
        ...
        description: createLocalizedText("生成 sprint 方案与范围边界。", "Generate the sprint plan and scope boundaries."),
        executor: {
          kind: "command",
          ref: "plan",
          command: "plan",
      ...
      },
      {
        ...
        description: createLocalizedText("把方案拆成任务卡、checklist 与 CSV 台账。", "Break the plan into task cards, checklist entries, and CSV records."),
        dependsOn: ["plan"],
        executor: {
          kind: "command",
          ref: "plan",
      ...
      },
      {
        ...
        description: createLocalizedText("根据任务卡推进代码和文档实现。", "Implement code and documentation changes against the task cards."),
        dependsOn: ["breakdown"],
        executor: {
          kind: "manual",
          ref: "implement"
      ...
      },
      {
        ...
        description: createLocalizedText("运行门禁、自测或 CI 等本地校验。", "Run local gates, self-checks, or CI-like validations."),
        dependsOn: ["implement"],
        executor: {
          kind: "command",
          ref: "check",
      ...
      },
      {
        ...
        description: createLocalizedText("生成带状态前缀的 code review 报告。", "Generate a status-prefixed code review report."),
      ...
      },
      {
        ...
        description: createLocalizedText("把复核结果追加到同一份 CR 文件并推进状态。", "Append verification results into the same review file and advance its status."),
        dependsOn: ["review"],
        executor: {
          kind: "command",
          ref: "review-verify",
      ...
      },
      {
        ...
        description: createLocalizedText("把执行与复核结果回写到 checklist 和 tasks.csv。", "Write execution and verification results back to checklist and tasks.csv."),
...
function applyStageOverrides(template, overrides, requireHumanApprovalFor) {
  const overrideMap = new Map((overrides ?? []).map((stageOverride) => [stageOverride.id, stageOverride]));
...
export function resolveWorkflowTemplate(workflowConfig = {}) {
  if ((workflowConfig.template ?? STANDARD_WORKFLOW_TEMPLATE_ID) !== STANDARD_WORKFLOW_TEMPLATE_ID) {
    throw new TypeError(`Unsupported workflow template: ${workflowConfig.template}`);
  }

  const resolvedTemplate = cloneValue(STANDARD_WORKFLOW_TEMPLATE);
...
export function listWorkflowStageIds(template = STANDARD_WORKFLOW_TEMPLATE) {
